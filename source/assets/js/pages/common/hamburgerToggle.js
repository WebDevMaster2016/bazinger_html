...
export default (hamburger, body, mainHeaderNav) => {
  ...
  const changeVisible = (boolean) => {
    ...
    const ariaLabelText = boolean
      ? `${window.front.translations.openMenu}`
      : `${window.front.translations.closeMenu}`;
  ...
  window.addEventListener(
    ...
    debounce(() => {
      if (
        body.classList.contains('disabled-scroll') &&
        window.innerWidth >= window.front.minWidth
      ) {
        changeVisible(true);
      }
    }),
