export default (arrImages) => {
  const removeDataImageSources = (target) =>
    ['data-src', 'data-srcset'].map((elem) => target.removeAttribute(elem));

  const setSrcData = (image) => {
    if (image.parentElement.nodeName === 'PICTURE') {
      const imageParents = Array.from(
        image.parentElement.querySelectorAll('source'),
      );
